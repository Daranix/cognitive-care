...
def main():
    # Example usage
    clinical_notes = """
    Patient presented with severe chest pain and shortness of breath for the past 2 weeks. 
    Reports intermittent headaches, pain scale of 7/10. Medical history includes type 2 diabetes 
    and hypertension. Has been experiencing mild nausea 3 times per day since starting new medication. 
    Prescribed metformin and lisinopril. Patient reported feeling better after taking the medications, 
    though occasional dizziness persists. Recommended follow-up appointment in 2 weeks.
    """
    
    analyzer = ClinicalNLPAnalyzer()
    results = analyzer.analyze_clinical_notes(clinical_notes)
    
    # Print results in a formatted way
    print("=== Extracted Symptoms ===")
    for symptom in results["symptoms"]:
        print(f"\nSymptom: {symptom}")
        #print(f"Severity: {symptom['severity']:.2f}")
